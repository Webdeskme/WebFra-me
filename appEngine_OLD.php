...
function ae_button($title, $test, $type, $text, $icon, $color, $id, $table, $con, $link){
  ...
  //if($title == $test){
    ...
    if ($type == 'save') {
      ...
      $wd_webHull->insert_data($db, $GLOBAL["table_name"], $con);
    ...
    elseif ($type == 'delete') {
      if($rpath == 'UsersFolder'){
        global $wd_appFile;
        if(is_file($wd_appFile . $path)){
        unlink($wd_appFile . $path);
        }
      }
      else{
        global $wd_appr;
        if(is_file($wd_appr . $path)){
        unlink($wd_appr . $path);
        }
      }
    }
